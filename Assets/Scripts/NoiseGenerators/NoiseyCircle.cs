...
{
    public static Vector2[] CreateNoiseyCircle(float radius, float noisePercentageOfRadius, float noiseStartPos, float scale, int numOfVertices){
        ...
        for(int i = 0; i < numOfVertices; i++){
            ...
            vertices[i] = new Vector2(Mathf.Cos(angle)*distanceVector.x - Mathf.Sin(angle)*distanceVector.y, Mathf.Sin(angle)*distanceVector.x + Mathf.Cos(angle)*distanceVector.y);
        }
